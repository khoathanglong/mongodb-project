...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        this.db.collection('item').aggregate([
            {$match:{} },
            {$group:{
                _id:"$category", num:{$sum:1}
            }},
            {$sort:{_id:1}}
        ]).toArray((err,categories)=>{
            assert.equal(null,err)
            let num=0;
            categories.forEach(each=>{num+=each.num})
            categories.unshift({_id:"All",num:num})
            callback(categories);
        })
    ...
    this.getNumItems = function(category, callback) {
      ...
      if(category=="All"){
            this.db.collection('item').find({}).toArray((err,docs)=>{
            let numItems=docs.length
            callback(numItems);
          })
      }else{
        this.db.collection('item').find({category:category}).toArray((err,docs)=>{
            let numItems=docs.length
            callback(numItems);
        })
      }
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
         ...
         *
         */
         this.db.collection('item').find({$text:{$search:query}})//have to create index "search" in the self first
            .skip(page*itemsPerPage)
            .limit(itemsPerPage)
            .toArray((err,docs)=>{
                assert.equal(null,err)
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        //Create text index in mongoshell
        //name of index is to delete it later
        // db.item.createIndex({
        //     title:"text",
        //     slogan:"text",
        //     description:"text"
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        let itemQuery = {_id:itemId}
        this.db.collection('item').updateOne(itemQuery,{$push:{reviews:reviewDoc}})
            .then(docs=>{callback(docs);console.log(docs)})
